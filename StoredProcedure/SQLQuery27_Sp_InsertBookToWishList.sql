...
CREATE     PROCEDURE [dbo].[InsertBookToWishList]
...
BEGIN TRANSACTION;
	...
	begin
		if((select count(*) from WishList where CustomerID = @CustomerID and BookID = @BookID) = 0)
		insert into WishList(CustomerID, BookID) values(@CustomerID, @BookID);
		else
		begin
			set @result = 3;
			throw 50005,'book already in wish list',-1
		end
	end
	else
	begin
